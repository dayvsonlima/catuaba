...
func TestActionValidation(t *testing.T) {
	t.Run("returns error when name is empty", func(t *testing.T) {
		...
		app := &cli.App{
			Commands: []*cli.Command{
				{Name: "scaffold", Action: Action},
			},
		}
		err := app.Run([]string{"catuaba", "scaffold"})
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "scaffold name is required")
	})
	...
	t.Run("returns error for invalid attribute format", func(t *testing.T) {
		...
		err := app.Run([]string{"catuaba", "scaffold", "Post", "badattr"})
		assert.Error(t, err)
